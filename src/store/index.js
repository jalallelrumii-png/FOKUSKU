...
export const useStore = create((set, get) => ({
  ...
  // ── Navigation ────────────────────────────────────────────
  page: 'onboarding',   // onboarding | timer | journal | stats | ai
  ...
  startTimer: () => {
    ...
    const id = setInterval(() => {
      ...
      if (s.secondsLeft <= 5 && s.secondsLeft > 1 && s.soundEnabled) {
        playTick()
      }
  ...
  completeSession: () => {
    ...
    get().sendNotif('Sesi Selesai! 🎉', `+${durationMins} menit fokus. Total hari ini: ${newFocus} menit.`)
    get().showToast(`Sesi selesai! +${durationMins} menit fokus 🎉`, 'success')

    if (mode === 'pomodoro' && newCount % 4 === 0) {
      setTimeout(() => get().showToast('4 sesi selesai! Istirahat panjang dulu 😌', 'info'), 2500)
    }
  },
